...
程序会合并整个电子书书架和笔记本列表，因此没有划线的书也会进入 Notion。定时任务每天更新阅读状态、时长、进度和阅读时间；只有新增书籍、该书自己的 `Sort` 游标变化或手动选择全量同步时，才会重新读取划线和个人点评正文。
...
写入 `作者`、`分类` 关联时，Notion Integration 还必须能够访问对应的关联数据库；无权限时程序会保留原关联并在日志中提示。

`阅读进度`、`阅读时长格式化` 等 Formula 字段由 Notion 自动计算，程序不会写入。年/月/周/日关系、豆瓣字段、章节/划线/读书笔记关系库没有对应的稳定微信读书字段或缺少子库结构，因此保留原值。实际划线与个人想法/点评会写入每本书页面的受管折叠区。微信读书书签目前只能取得数量，Gateway 不提供书签正文。
...
2. `existing-page-mode=append` 会在旧正文后追加受管同步区域；以后只替换该区域，这是本分支默认值。
3. `existing-page-mode=preserve` 只更新属性，完全不添加受管区域。
4. 可以填写 `book-id`，先只测试一本书；留空时处理所有书籍。

建议先填一本书的 `book-id`，使用 `full-sync=true`、`existing-page-mode=append` 测试。确认字段和正文无误后，清空 `book-id` 再按相同选项运行一次全量同步。旧正文不会自动删除；如果旧程序已经写过一份划线，可能需要人工清理一次旧内容。
